...
function navLink(p, current) {
  const active = p.title === current ? ' active' : '';
  return `      <li>
        <a href="${p.file}" class="${active.trim()}">
...
function template(page) {
  const navLinks = PAGES.map(p => navLink(p, page.title)).join('\n');
  return `<!DOCTYPE html>
<html lang="tr">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>${page.title} — La Foa</title>
...
<header class="site-header">
  <div class="header-content">
    ...
    <a href="index.html" class="logo-area">
      <div class="logo-icon">
        <img src="assets/lafoa-logo-clean.png" alt="La Foa" class="logo-img">
      </div>
      <div class="logo-text">
        <h1>Late Antique Feudal Opposition in Armenia</h1>
      </div>
    </a>
...
${navLinks}
    ...
    <button class="nav-hamburger" onclick="document.querySelector('.nav-links').classList.toggle('open')" aria-label="Menü">
...
<script>loadSettlementContent('${page.slug}');</script>
</body>
</html>
`;
}
...
for (const page of PAGES) {
  const outPath = path.join(__dirname, '..', page.file);
